...
author: "Thibaut Jombart, Emily Nightingale, Quentin Leclerc, Sam Abbott, ... for CMMID"
...
## Approach 1 implementation: model and deviations
...
get_stats <- function(dates, counts, k = 1, n_sim = 1000) {
  ...
  ## 'Fringe' index (made-up name)
  ##
  ## Compares the value of an observation to a reference empirical distribution;
  ## it has value:
  ## - 0 when an observation is exactly the median of the empirical distribution
  ## - 1 when it is outside the range of the distribution
  ## - intermediate values increase with the proportion of the distribution less
  ##   'extreme' than the observation (p(sim < obs) if (obs > median), and p(sim >
  ##   obs) if (obs < median)

  get_fringe <- function(obs, sim) {
    2 * abs(mean(obs > sim, na.rm = TRUE) - 0.5)
  }


  ## 'IQRD' index (also made-up)
  ##
  ...
  n <- length(dates)
  res_resid <- numeric(n)
  res_fringe <- numeric(n)
  res_iqrd <- numeric(n)

  ## calculate the different statistics
  for (i in seq_len(n)) {
    res_resid[i] <- get_resid(obs = res$obs$counts[i], sim = res$pred[[i]])
    res_fringe[i] <- get_fringe(obs = res$obs$counts[i], sim = res$pred[[i]])
    res_iqrd[i] <- get_iqrd(obs = res$obs$counts[i], sim = res$pred[[i]])
    res_mad[i] <- get_mad(obs = res$obs$counts[i], sim = res$pred[[i]])
  }
  ...
  res_combined <- (rescale(res_resid) +
    rescale(res_fringe) +
    rescale(res_iqrd)) / 3
  
  cbind.data.frame(dates = dates,
                   counts = counts,
                   resid = res_resid,
                   fringe = res_fringe,
                   iqrd = res_iqrd,
...
res_stationary_long <- res_stationary %>%
  pivot_longer(-(1:2), names_to = "metric", values_to = "value")

res_stationary_long %>%
  ggplot(aes(x = dates, y = value, color = metric)) +
...
res_stationary_increase_long <- res_stationary_increase %>%
  pivot_longer(-(1:2), names_to = "metric", values_to = "value")

res_stationary_increase_long %>%
  ggplot(aes(x = dates, y = value, color = metric)) +
  facet_wrap( ~ metric, scales = "free") +
  geom_vline(xintercept = 25, linetype = 2) +
  theme_bw() +
...
res_decrease_increase_long <- res_decrease_increase %>%
  pivot_longer(-(1:2), names_to = "metric", values_to = "value")

res_decrease_increase_long %>%
  ggplot(aes(x = dates, y = value, color = metric)) +
  facet_wrap( ~ metric, scales = "free") +
  geom_vline(xintercept = 25, linetype = 2) +
  theme_bw() +

...
find_outliers <- function(fitted_model, newdata, alpha = 0.05) {
  pred_ci <- ciTools::add_ci(tb = newdata, fit = fitted_model,
...
detect_epichange <- function(x, models, day = 1, count = 2,
  ...
  best_model <- select_model(x_train, models)
  fit <- best_model$model(x_train)
  diagnostics <- find_outliers(fitted_model = fit, x_test, alpha = alpha)

  list(results = diagnostics,
       all_models = models,

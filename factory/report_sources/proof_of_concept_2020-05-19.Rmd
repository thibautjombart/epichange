...
get_stats <- function(dates, counts, k = 1, n_sim = 1000, C = 2) {
  ...
  for (i in seq_len(n)) {
    ...
    stats$fringe[i] <- get_fringe(obs = res$obs$counts[i], sim = res$pred[[i]])
...
## neg-bin models
model_nb_constant <- function(data) {
  .validate_data(data)
  MASS::glm.nb(data = data, formula = count ~ 1)
}
...
## neg-bin models
model_qp_constant <- function(data) {
  .validate_data(data)
  glm(data = data, formula = count ~ 1, family = "quasipoisson")
}
...
#  list(name = "poisson_time_weekday", model = model_poisson_time_weekday)
  list(name = "nb_constant", model = model_nb_constant),
  list(name = "nb_time", model = model_nb_time)
#  list(name = "nb_time_weekday", model = model_nb_time_weekday)
#  list(name = "qp_constant", model = model_qp_constant),
#  list(name = "qp_time", model = model_qp_time)
#  list(name = "qp_time_weekday", model = model_qp_time_weekday)
)



select_model <- function(x, k = 1, models) {
  ...
  ## fit models and apply cross-validation current re-sampling uses Jackniffe;
  ## see ?rsample::vfold_cv for more options

  models_mrse_values <- vapply(models,
                        function(model) jack_rmse(x_train, model),
                        numeric(1))
  ...
  ## return best model
  models[[which.min(models_mrse_values)]]
